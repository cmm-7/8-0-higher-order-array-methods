...
function getSongsBySaib(songs) {
  return songs.filter(song => song.artist === "Saib").map(song => song);
}
...
function getSongsOverThreeMinutes(songs) {
  return songs.filter(song => song.runtimeInSeconds > 180).map(song => song);
}
...
function getTitleTracks(songs) {
  return songs.filter(song => song.title === song.album).map(song => song);
}
